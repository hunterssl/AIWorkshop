  ...
  const send = async (content, stream = true, chatOptions = {}) => {
    ...
    try {
      ...
      if (stream) {
        ...
        // 使用 modelStore 获取完整 URL（安全解析，回退到 getBaseUrl）
        ...
        // 非流式：使用 axios（request.js 拦截器自动处理 API Key 和渠道）
        const response = await chatCompletions(adaptedParams)
        // 适配响应数据
        const adaptedData = adaptResponse('chat', response)
        const reply = adaptedData?.choices?.[0]?.message?.content || ''
        messages.value.push({ role: 'user', content })
...
export const useImageGeneration = () => {
   ...
   */
  const generate = async (params, signal) => {
    setLoading(true)
    images.value = []
    ...
    try {
      ...
      const requestData = {
        ...
        size: params.size || modelConfig?.defaultParams?.size || '2048*2048',
        // n: params.n || 1
      }
...
export const useVideoGeneration = () => {
   ...
   */
  const createVideoTaskOnly = async (params, signal) => {
    const modelConfig = getModelByName(params.model)
