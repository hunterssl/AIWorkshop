...
// ========== 节点默认尺寸 | Node default dimensions ==========
const NODE_DEFAULT_DIMS = {
...
// ========== 节点四角坐标辅助函数 | Node corner helpers ==========

/**
 * 获取节点的四个角顶点坐标 | Get all four corner coordinates
 */
export const getNodeCorners = (node) => {
  const dims = NODE_DEFAULT_DIMS[node.type] || { width: 250, height: 250 }
...
/**
 * 获取节点右侧 x 坐标 | Get right edge x-coordinate
 */
export const getNodeRight = (node) => {
  const dims = NODE_DEFAULT_DIMS[node.type] || { width: 250, height: 250 }
  const w = node.width || dims.width
  return (node.position?.x ?? 0) + w
}

/**
 * 获取节点底部 y 坐标 | Get bottom edge y-coordinate
 */
export const getNodeBottom = (node) => {
  const dims = NODE_DEFAULT_DIMS[node.type] || { width: 250, height: 250 }
  const h = node.height || dims.height
  return (node.position?.y ?? 0) + h
}
...
export const addNode = (type, position = { x: 100, y: 100 }, data = {}) => {
  const id = getNodeId()
  const now = Date.now()
  const dims = NODE_DEFAULT_DIMS[type] || { width: 250, height: 250 }
  const newNode = {
    id,
    type,
    position,
...
export const updateNode = (id, updates) => {
  const nodeLevelKeys = ['width', 'height', 'zIndex', 'position']
  nodes.value = nodes.value.map(node => {
    if (node.id !== id) return node
    const nodeUpdates = {}
    const dataUpdates = {}
    for (const [key, val] of Object.entries(updates)) {
      if (nodeLevelKeys.includes(key)) {
        nodeUpdates[key] = val
      } else {
        dataUpdates[key] = val
      }
...
export const loadProject = (projectId) => {
  ...
  if (canvasData) {
    ...
    // 向后兼容：为旧节点补充 width/height | backward compat: add width/height to old nodes
    nodes.value = nodes.value.map(node => {
      if (node.width == null || node.height == null) {
        const dims = NODE_DEFAULT_DIMS[node.type] || { width: 250, height: 250 }
        return { ...node, width: node.width || dims.width, height: node.height || dims.height }
      }
      return node
    })
