  ...
  <div class="llm-node-wrapper" @mouseenter="showHandleMenu = true" @mouseleave="showHandleMenu = false">
    ...
    <div
      ...
      <div class="p-3 space-y-3">
        ...
        <button @click="handleGenerate" :disabled="false" :class="isExecuting ? 'bg-red-500 hover:bg-red-600' : 'bg-purple-500 hover:bg-purple-600'" :title="isExecuting ? '点击中断执行' : '执行生成'"
          class="w-full px-4 py-2 text-sm rounded-lg bg-purple-500 hover:bg-purple-600 text-white transition-colors flex items-center justify-center gap-2">
          <n-spin v-if="isExecuting" :size="14" />
          <n-icon v-else :size="14">
            <SparklesOutline />
          </n-icon>
          {{ isExecuting ? '执行中... 点击中断' : '执行生成' }}
        </button>
